...
//******************************************************************************
  /* loadAnnotations in IDS. Input: the name of an XML file */
...
test on the models input = all the features
store probability
*/
vector<vector<double> > TestScene::predictObjectClasses() {

  int fsize = featureListSingleObject[0].size();
  ...
  for (int i  = 0; i < featureListSingleObject.size(); i++ ) {  
    ...
    for(vector<cv::EM>::iterator it2 = learnedModelSingleObject.begin(); it2 != learnedModelSingleObject.end(); ++it2) {
      ...
      */
      if (TESTFLAG) {
        cout << "The extracted features test object " << i << " is " << endl
            << featsTrain << endl  << endl;
      }
...
//*************************************************************************************
/* New strategy for similarity score computation */
